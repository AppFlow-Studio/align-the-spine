-- Additive to 202608180002. The editorial admin form (components/admin/
-- editorial-form.tsx) only ever let editors save the "core" fields
-- (title/excerpt/directAnswer/keyTakeaways/faqs/blocks/seoTitle/
-- metaDescription) even though content_items has several more editable
-- scalar columns already: featured, medical_review_required, noindex,
-- noindex_reason, og_title, og_description. None of those needed a schema
-- change (they already exist on the table from 202608160001) — only
-- save_content_draft's UPDATE list was incomplete. Re-created here with
-- the same body otherwise, plus the new SET clauses.
--
-- Deliberately NOT in scope here: categories/tags/related content (live in
-- separate join tables — content_categories/content_tags/content_relations
-- — and need proper diff/upsert logic, not a single SET clause), sources
-- (same join-table shape via content_sources), and clinician review
-- (clinician_reviewer_id is a FK to profiles, not free text — needs a
-- reviewer picker, not a text input). Those remain a follow-up.
