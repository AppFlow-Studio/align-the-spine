...
- **Hero's negative top margin** (to bleed its background under the fixed `Navbar`, over `TopStatsBar`) is computed from `TopStatsBar`'s actual rendered height at each breakpoint, not guessed. `components/layout/top-stats-bar.tsx` renders `siteConfig.stats` (5 items) in a `dl.grid` with `text-stat-label` (18px/28px line-height) over `text-stat-value` (32px/40px line-height), `gap-1` (4px) between them, `gap-y-8` (32px) between rows, wrapped in `.container.py-4.md:py-6`. Row height ≈ 28+4+40 = 72px. Grid columns: `grid-cols-2` (<640px) → `sm:grid-cols-3` (640–1023px) → `lg:grid-cols-5` (≥1024px). Padding: `py-4` (32px total) below `md` (768px), `md:py-6` (48px total) at/above it. This gives:
  ...
  | So Hero's top margin was _originally computed on paper_ as `-mt-[312px] sm:-mt-[208px] md:-mt-[224px] lg:-mt-[120px]`, assuming exactly 5 short, non-wrapping labels. **These values were superseded during implementation/review**: real-browser measurement found labels actually wrap within some widths (the "non-wrapping" assumption above didn't hold), so the shipped values are `-mt-[516px] sm:-mt-[304px] md:-mt-[240px] lg:-mt-[176px]` (each tier's real worst-case rendered height + 16px buffer, verified via dense sampling). See `.superpowers/sdd/progress.md` and `.superpowers/sdd/task-2-report.md` for the full derivation and correction history. Same caveat still applies: if `siteConfig.stats` content changes enough to alter wrapping/row-count, these four values need remeasuring the same way — do not guess them from arithmetic alone. |
...
export function Hero({
  ...
  return (
    // NOTE: shipped values differ from this plan's original draft — see the
    // Global Constraints table above and .superpowers/sdd/task-2-report.md.
