...
| `lead_sensitive_payloads` | AES-256-GCM ciphertext, IV, tag, key version, and field-name manifest   | Service role only; not exposed in CRM UI                         |
| `lead_consent_receipts`   | Exact wording/version/channel/granted state at submission               | Append-only through ingestion                                    |
| `lead_status_events`      | CRM status timeline with actor and reason                               | Reasons must not contain clinical detail                         |
