...
export interface TypesGridItem {
  ...
  /** Renders inside a highlighted, boxed row with a trailing arrow instead
   * of the plain stacked text — matches the Figma "Chronic" treatment,
   * which sits in its own callout rather than looking like the other 5
   * types. */
  highlighted?: boolean;
}
 ...
 * descriptions carry their own inline links to specific phrases. The last
 * item renders as a highlighted callout row instead of plain text. */
        ...
        <div className="grid grid-cols-1 gap-x-16 gap-y-10 border-t border-mute-300 pt-10 md:grid-cols-2">
          {items.map((item) =>
            item.highlighted ? (
              <div
                key={item.name}
                className="flex items-center justify-between gap-4 bg-overlay-teal-12 p-6"
              >
