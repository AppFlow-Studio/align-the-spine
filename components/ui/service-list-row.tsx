...
export function ServiceListRow({ item, className, locale = DEFAULT_LOCALE }: ServiceListRowProps) {
      ...
      <div className="flex min-w-0 flex-1 flex-col items-start gap-4">
        ...
        <p className="font-sans text-card-body mt-4">
          <span className="text-ink-500">
            {item.duration} | {copy.contact} {siteConfig.business.phone}
          </span>
          <br />
        ...
        <div className="mt-auto flex w-full items-center justify-between gap-4 self-end">
          ...
          <Button variant="book" href={bookingCta.href} className="text-lg">
            {copy.book}
          </Button>
        </div>
      </div>
