...
import { htReviewsPage } from "@/content/ht/pages";
...
const route = getHtRoute("/ht/komante-pasyan");

export const metadata: Metadata = buildHtRouteMetadata(route);

/** /ht/komante-pasyan — Haitian Creole counterpart of /reviews and
 * /pt/avaliacoes (ATS-SEO-136). Reviews render verbatim, in whatever
 * language each patient wrote them — never translated (see
 * htReviewsPage.languageNote for why). No AggregateRating markup here
 * either, exactly as every other language's page: the 5.0/164 figure is a
 * client-asserted number rather than one read from a live source, and
 * rating markup is a stronger, machine-consumed claim than showing the
 * same figure as copy.
 */
const breadcrumbs = [
  { name: "Paj Prensipal", path: "/ht" },
  { name: "Kòmantè", path: route.path },
];

export default function HtReviewsPage() {
  const reviews = siteConfig.reviewsRating;
  const otherStats = getLocalizedStats("ht").filter((stat) => stat.label !== "Kòmantè");
    ...
    <>
      ...
      <section className="relative flex flex-col overflow-hidden pt-10 lg:-mt-[176px] lg:min-h-[860px] lg:flex-row">
          ...
          <Container>
            <div className="container relative z-10 flex h-full flex-col justify-start pb-16 pt-[120px] lg:pb-16 lg:pr-12 lg:pt-[170px]">
              <div className="max-w-lg">
                ...
                <h1 className="font-display text-5xl font-medium text-white">{htReviewsPage.h1}</h1>
                <p className="mt-4 font-sans text-body-lg text-white">{htReviewsPage.intro}</p>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <span className="font-sans text-body-lg text-white">
                        <strong className="font-semibold">
                          {reviews.value.count} {htReviewsPage.ratingSuffix}
                        </strong>{" "}
                        {htReviewsPage.ratingTail}
                      </span>
                    </div>
                  </div>
                )}
        ...
        <div className="relative flex flex-col bg-navy-900 px-6 pb-16 pt-10 sm:px-10 lg:w-[640px] lg:shrink-0 lg:px-16 lg:pb-16 lg:pt-[170px] xl:w-[760px] 2xl:w-[800px]">
          <LeadForm
            locale="ht"
            heading={htReviewsPage.formHeading}
            variant={htLeadFormVariants.reviewsEval.variant}
            fields={htLeadFormVariants.reviewsEval.fields}
            submitLabel={htLeadFormVariants.reviewsEval.submitLabel}
            submitVariant="teal"
            fieldOutline
            labelCase="none"
            ...
            className="gap-y-4"
          />
          <p className="mt-6 font-sans text-body-lg text-mute-300">{htReviewsPage.formFootnote}</p>
        </div>
      </section>

      <Section spacing="lg" className="container">
        <div className="flex flex-col gap-8">
          <SectionHeading eyebrow="Kòmantè" sub={htReviewsPage.languageNote}>
            {htReviewsPage.carouselHeading}
          </SectionHeading>
          <ReviewsCarousel reviews={testimonials} locale="ht" />
        </div>
      </Section>
