...
import { htServiceAreasCopy } from "@/content/ht/service-areas";
...
const route = getHtRoute("/ht/zon-nou-sevi");

export const metadata: Metadata = buildHtRouteMetadata(route);

/** /ht/zon-nou-sevi — Haitian Creole counterpart of /service-areas and
 * /pt/areas-de-atendimento (ATS-SEO-136).
 *
 * Deliberately NOT a reuse of components/content/service-area-hero.tsx and
 * its nineteen-community grid, same reasoning app/(pt)/pt/areas-de-atendimento/page.tsx
 * documents: that component and content/es/service-areas.ts are built
 * entirely around content/es/service-areas-cities.ts's nineteen individual
 * Spanish city pages, none of which have a Haitian Creole counterpart (out
 * of this ticket's scope — see content/ht/seo.ts's header comment).
 * Building a nineteen-city grid here with no pages behind any of the links
 * would be exactly the "empty/partial page for route parity" this
 * ticket's acceptance criteria rule out. Instead this page states the one
 * verified office plainly and explains, honestly, how home-visit
 * eligibility for nearby communities is confirmed (by phone, case by
 * case) — the real, current process, not a fabricated coverage map.
 */
export default function HtServiceAreasPage() {
  const breadcrumbs = [
    { name: "Paj Prensipal", path: "/ht" },
    { name: htServiceAreasCopy.breadcrumb, path: route.path },
  ];
  const { address, phone, phoneHref } = siteConfig.business;
  const { hero, intro, office } = htServiceAreasCopy;
